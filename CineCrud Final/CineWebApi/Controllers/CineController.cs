    ...
    public class CineController : ControllerBase
    {
        ...
        public IActionResult PostComprobante(Comprobante oC)
        {
            ...
            if (serv.SaveComprobante(oC) == true)
                return Ok("¡Enhorabuena, comprobante registrado con exito! ¡Estas a solo unos pasos de salir de Salita de 4! ");
            else
        ...
        public IActionResult CargarCliente(List<Parametro> lParams)
        {
            if (lParams == null || lParams.Count == 0)
                return BadRequest("Se requiere una lista de parametros (id)");

            return Ok(serv.CargarCliente(lParams));
        }
        ...
        [HttpPost("CargarComprobantes")]
        public IActionResult CargarComprobantes(List<Parametro> lParams)
        {
            if (lParams == null || lParams.Count == 0)
                return BadRequest("Se requiere una lista de parametros");

            return Ok(serv.CargarComprobantes(lParams));
        }
        ...
        public IActionResult ActualizarComprobante(Comprobante oC)
        {
            if (oC == null)
                return BadRequest("");

            if (serv.ActualizarComprobante(oC) == true)
                return Ok("¡Comprobante actualizado con exito!");
            else
                return Ok("¡Error al actualizar comprobante!");
        }
        ...
        public IActionResult PutButaca(Comprobante oC)
        {
            ...
            if (serv.ActualizarButaca(oC) == true)
                ...
                return Ok("Oh, genial, cometiste el mismo error al actualizar las butacas ¡Parece ser tu talento especial!");
        }
