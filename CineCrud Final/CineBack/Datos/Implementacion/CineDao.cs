...
namespace CineBack.Datos.Implementacion
{
    internal class CineDao : ICineDao
    {
        ...
        public Comprobante CargarComprobante(List<Parametro> lParams)
        {
            ...
            foreach (DataRow row in table.Rows)
            {
                oC.Id = Convert.ToInt32(row["id_comprobante"].ToString());
...
return rst;
        ...
        {
            DataTable table = HelperDao.GetInstance().GetTable("sp_consultar_comprobantes", lParams);
            ...
            foreach (DataRow row in table.Rows)
            {
                ...
                oC.Id = Convert.ToInt32(row["id_comprobante"].ToString());
                oC.Fecha = Convert.ToDateTime(row["fecha"].ToString());
                oC.Cliente.Id = row["id_cliente"] == DBNull.Value ? null : Convert.ToInt32(row["id_cliente"]);
                oC.Cliente.Nombre = row["nombre"].ToString();
                oC.Cliente.Apellido = row["apellido"].ToString();
                oC.CantEntradas = Convert.ToInt32(row["cant_entradas"].ToString());
                oC.PrecioTotal = row["precio_total"] == DBNull.Value ? null : Convert.ToDouble(row["precio_total"].ToString());

                lComprobantes.Add(oC);
            }
        ...
        public bool ActualizarComprobante(Comprobante oC)
        {
            ...
            try
            {
                cnn.Open();
                txn = cnn.BeginTransaction();
                SqlCommand cmd = new SqlCommand("sp_actualizar_comprobante", cnn, txn);
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.AddWithValue("@id_comprobante", oC.Id);
                cmd.ExecuteNonQuery();
                ...
                foreach (DetalleComprobante oDC in oC.DetsComprobante)
                {
                    ...
                    cmdDC.Parameters.AddWithValue("@precio_entrada", oDC.PrecioEntrada);
                    cmdDC.Parameters.AddWithValue("@id_comprobante", oC.Id);
                    cmdDC.Parameters.AddWithValue("@id_butaca", oDC.Butaca.Id);
